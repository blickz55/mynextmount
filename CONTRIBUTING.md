...
**Scaling guides + digests + tips to the full catalog:** **`docs/guide-experience-roadmap.md`**. Progress metric: **`npm run data:guide-experience`**. **Automated batch generation** is **allowed** (maintainer responsibility for ToU — **`docs/data-harvesting.md`** *Maintainer override*, **`.cursorrules`**). Guides: **`npm run content:guides-batch`** (see **`docs/guide-experience-roadmap.md`**).
