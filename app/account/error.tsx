...
export default function AccountError({
  ...
  return (
    <main id="main-content" tabIndex={-1} className="app-main app-shell">
      ...
      <p className="lead">
        My Mounts glitched while loading. Usually a quick server burp — your
        saved mounts are probably still there.
      </p>
      {error.digest != null && error.digest !== "" && (
        <p className="field-hint">Error id: {error.digest}</p>
      )}
      ...
      <p className="field-hint">
        If Try again loops, hit Reload My Mounts or open the tool and use{" "}
        <strong>Reload from account</strong> after sign-in.
      </p>
    </main>
  );
}
