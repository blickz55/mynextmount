/**
 * Mirrors lib/mountLoreTheme.ts for batch scripts (theme hint → LLM).
 * Keep in sync when editing heuristics.
 */
...
export function inferMountLoreTheme(m) {
  const exp = String(m.expansion || "").toLowerCase();
  ...
  if (exp.includes("burning crusade") || exp.includes("outland"))
    return "outland";
  if (exp.includes("pandaria") || exp.includes("mists")) return "pandaria";
  if (exp.includes("legion") || exp.includes("broken isles"))
    return "brokenIsles";
  if (exp.includes("battle for azeroth") || exp.includes("zandalar"))
    return "zandalar";
  ...
  if (/tiragarde|drustvar|stormsong|kul tiras|proudmoore/.test(hay))
    return "kulTiras";
  if (/fel|legion|demon|infernal|mannoroth|burning legion|eredar|pit lord/.test(hay))
    return "fel";
...
export function loreThemeWithRotation(m, spellId) {
...
const HINTS = {
  ...
  shadowlands:
    "death-magic, bone ash, oath-bound souls, cold that eats sound",
  ...
  dragonflight:
    "bronze winds, titan silence, scales catching impossible sun",
...
export function themeHintForModel(theme) {
